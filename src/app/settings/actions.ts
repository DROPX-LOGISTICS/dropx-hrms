...
export async function saveAttendanceSettings(formData: FormData) {
  ...
  if (!supabaseAdmin) redirect("/settings/attendance-policy?error=Database%20configuration%20is%20missing");
  ...
  if (!workWeek.length || !Number.isInteger(grace) || grace < 0 || grace > 180 || !Number.isInteger(fullDay) || fullDay < 60 || !Number.isInteger(halfDay) || halfDay < 30) {
    redirect("/settings/attendance-policy?error=Enter%20valid%20attendance%20settings");
  }
  ...
  if (error) redirect(`/settings/attendance-policy?error=${encodeURIComponent(error.message)}`);
  revalidatePath("/settings");
  revalidatePath("/settings/attendance-policy");
  redirect("/settings/attendance-policy?notice=Attendance%20policy%20saved");
}
