...
export async function setEmployeeActive(_prev: ActionFeedbackState, formData: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("people.manage");
  ...
  return actionSuccess(`Employee ${nextActive ? "activated" : "deactivated"}.`);
}
