...
export default async function ContractorPage({ params, searchParams }: {
  ...
  const packages = canProcessPayroll ? await listPackagesForPayee(auth, "contractor", contractor.id) : [];
  ...
  return <>
    ...
    {canProcessPayroll ? <section className="panel" style={{ marginTop: 16 }}>
      <div className="panel-head"><div><h2>Job &amp; package pay</h2><p className="panel-subtitle">Add one-off job or package pay entries for this contractor. Entries are pulled into the next payroll run covering their job date.</p></div></div>
      <div className="panel-body">
        <form action={addPayPackageAction} className="form-grid">
          <input type="hidden" name="payee" value={`contractor:${contractor.id}`} />
          <input type="hidden" name="redirect_to" value={`/people/contractors/${contractor.id}`} />
          <div className="field"><label htmlFor="ctr-package-title">Job / package title *</label><input id="ctr-package-title" name="title" placeholder="e.g. Delivery batch – 20 Jul" required /></div>
          <div className="field"><label htmlFor="ctr-package-amount">Amount (₹) *</label><input id="ctr-package-amount" name="amount" type="number" min="0.01" step="0.01" required /></div>
          <div className="field"><label htmlFor="ctr-package-date">Job date *</label><input id="ctr-package-date" name="job_date" type="date" required /></div>
          <div className="field wide"><label htmlFor="ctr-package-description">Notes</label><input id="ctr-package-description" name="description" placeholder="Optional" /></div>
      ...
      <div className="table-wrap"><table><thead><tr><th>Title</th><th>Job date</th><th>Amount</th><th>Status</th><th>Action</th></tr></thead><tbody>
        {packages.length ? packages.map((item) => <tr key={item.id}>
          ...
          <td>{item.status === "draft" || item.status === "approved" ? <form action={deletePayPackageAction}><input type="hidden" name="package_id" value={item.id} /><input type="hidden" name="redirect_to" value={`/people/contractors/${contractor.id}`} /><SubmitButton className="button secondary small" pendingLabel="Removing…">Remove</SubmitButton></form> : null}</td>
