...
import { getExitCaseStats, listExitCases, listExitEligibleEmployees, loadExitMasters } from "@/lib/exit-data";
import { can } from "@/lib/permissions";
import { createTerminationCase } from "./actions";

export const metadata: Metadata = { title: "Exit Management" };
export default async function ExitsPage({ searchParams }: { searchParams?: { create?: string; error?: string; notice?: string; search?: string; status?: string; scenario?: string; page?: string } }) {
  const auth = await requireHrmsAuth("exit.view");
  const manage = can(auth.permissions, "exit.manage");
  const creating = searchParams?.create === "termination" && manage;
  const [{ rows: cases, total, page, pageSize }, stats, employees, masters] = await Promise.all([
    listExitCases(auth, searchParams),
    getExitCaseStats(auth),
    creating ? listExitEligibleEmployees(auth) : Promise.resolve([]),
    creating ? loadExitMasters(auth) : Promise.resolve(null)
  ]);
  const { open, clearance, ready, closed } = stats;
  ...
  return <>
    ...
    {creating ? <section className="panel employee-form-panel">
    ...
    <section className="panel">
      ...
      <Pagination page={page} pageSize={pageSize} total={total} basePath="/exits" searchParams={searchParams} />
    </section>
  </>;
}
