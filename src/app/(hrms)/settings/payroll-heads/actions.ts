...
function fail(error: unknown): ActionFeedbackState {
  return actionError(error instanceof Error ? error.message : "Unable to save payroll head.");
}
...
export async function createPayrollHead(_prev: ActionFeedbackState, formData: FormData): Promise<ActionFeedbackState> {
  ...
  return finish("Payroll head created.");
}

export async function savePayrollHead(_prev: ActionFeedbackState, formData: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Payroll head updated.");
}

export async function togglePayrollHead(_prev: ActionFeedbackState, formData: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
    const id = value(formData, "id");
  ...
  return finish("Payroll head status updated.");
}
