...
function finish(message: string) { revalidatePath("/settings/exit"); revalidatePath("/exits"); return actionSuccess(message); }
function fail(error: unknown): ActionFeedbackState { return actionError(error instanceof Error ? error.message : "Unable to save master data."); }
...
export async function saveExitPolicy(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  ...
  return finish("Exit policy saved.");
}

export async function saveExitReason(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Exit reason saved.");
}

export async function saveExitWorkflow(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Approval workflow saved.");
}

export async function saveExitTaskTemplate(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Exit task template saved.");
}

export async function saveExitDocumentTemplate(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Document template saved.");
}

export async function saveExitNotificationTemplate(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Notification rule saved.");
}

export async function toggleExitMaster(_prev: ActionFeedbackState, form: FormData): Promise<ActionFeedbackState> {
  const auth = await requireHrmsAuth("settings.manage");
  try {
  ...
  return finish("Master status updated.");
}
