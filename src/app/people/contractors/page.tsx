...
export default async function ContractorsPage({ searchParams }: {
  searchParams?: { add?: string; error?: string; notice?: string; search?: string; status?: string; location?: string };
}) {
  ...
  const [contractors, locations, designations] = await Promise.all([
