...
export type LeaveRequestRow = { id: string; employee_id: string; leave_type_id: string; start_date: string; end_date: string; days: number; reason: string; status: string; requested_at: string; reviewed_at: string | null; reviewer_note: string | null; employees?: { employee_code: string | null; full_name: string } | null; hr_leave_types?: { name: string; code: string; color: string } | null };
export type OverviewActivityRow = { id: string; start_date: string; end_date: string; status: string; requested_at: string; employees?: { full_name: string } | null; hr_leave_types?: { name: string } | null };
...
export async function listEmployees(auth: HrmsAuthContext, filters?: { status?: string; search?: string; location?: string }) {
  ...
  if (!auth.allLocations) query = query.in("location_id", auth.locationIds.length ? auth.locationIds : ["00000000-0000-0000-0000-000000000000"]);
  if (filters?.status === "active") query = query.eq("is_active", true);
  if (filters?.status === "inactive") query = query.eq("is_active", false);
  if (filters?.location) query = query.eq("location_id", filters.location);
  if (filters?.search) query = query.or(`full_name.ilike.%${filters.search.replaceAll(",", "")}%,employee_code.ilike.%${filters.search.replaceAll(",", "")}%,mobile.ilike.%${filters.search.replaceAll(",", "")}%`);
...
export async function listLeaveRequests(auth: HrmsAuthContext, status?: string) {
  let query = requireAdmin().from("hr_leave_requests").select("id, employee_id, leave_type_id, start_date, end_date, days, reason, status, requested_at, reviewed_at, reviewer_note, employees(employee_code,full_name), hr_leave_types(name,code,color)").eq("company_id", auth.companyId).order("requested_at", { ascending: false });
...
export async function loadOverview(auth: HrmsAuthContext) {
  ...
  if (!auth.allLocations) {
    const locationIds = auth.locationIds.length ? auth.locationIds : ["00000000-0000-0000-0000-000000000000"];
  ...
  const [employeeResult, attendanceResult, pendingResult, recentResult] = await Promise.all([
    employees,
    attendance,
    admin.from("hr_leave_requests").select("id", { count: "exact", head: true }).eq("company_id", auth.companyId).eq("status", "pending"),
    admin.from("hr_leave_requests").select("id, start_date, end_date, status, requested_at, employees(full_name), hr_leave_types(name)").eq("company_id", auth.companyId).order("requested_at", { ascending: false }).limit(5)
  ]);
