...
export async function listContractors(auth: HrmsAuthContext, filters?: { status?: string; search?: string; location?: string }) {
  let query = requireAdmin()
    .from("contractors")
    .select("id, dropx_id, biometric_id, full_name, mobile, mobile_country_code, email, date_of_join, location_id, designation, onboarding_status, profile_return_remarks, profile_photo_path, is_active, stations(station_code,station_name)")
  ...
  const { data, error } = await query;
  if (error) throw new Error(error.message);
  return withProfilePhotoUrls((data ?? []) as unknown as ContractorRow[]);
}
